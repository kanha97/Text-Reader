package com.devkanhaiya.bookreader.util
...
import com.ewayapp.util.FileUri
...
class FilePickUtils : LifeCycleCallBackManager {
    ...
    fun selectImageFromCamera() {
        ...
        if (activity != null) {
            fileUri = AppUtil.createImageFile(activity!!, "CAMERA")
        }
        if (fileUri == null) {
            return
        }
...
*/
        ...
        val filename = activity?.let { AppUtil.createImageFile(it, "")!!.file?.absolutePath }
        try {
    ...
    private fun getRealPathFromURI(contentURI: String): String {
        ...
        return if (cursor == null) {
            ...
            } else "${AppUtil.getWorkingDirectory()}/${cursor.getString(0)}"
        }
    }
    ...
    private fun performCrop(uri: Uri?) {
        val cropFile = activity?.let { AppUtil.createImageFile(it, "CROP") }
        if (fragment != null) {
    ...
    companion object {
        ...
        const val CAMERA_PICTURE = 10
        const val GALLERY_PICTURE = 11
